...
// Assuming this is part of your form submission processing
$payMethod = isset($_POST['pay_method']) ? $_POST['pay_method'] : null;

if ($payMethod === 'Cash') {
    // The command to open the cash register
    $command = 'echo -e "\x1B\x70\x00\x19\xFA" > /dev/ttyUSB0';  // Adjust according to your device
    ...
    if ($return_var === 0) {
        echo "Cash register command executed successfully.";
    } else {
        echo "Failed to execute cash register command. Return value: " . $return_var;
    }

    // Optionally, print the output array for debugging
    print_r($output);

    // Alternatively, log the results to a file
    $logFile = './cash_register_log.txt';
    $logMessage = date('Y-m-d H:i:s') . " - Command executed. Return value: " . $return_var . "\nOutput: " . implode("\n", $output) . "\n\n";
...
$('form').on('submit', function(event) {
    ...
    function updateTotalForExistingRow(row) {
        ...
        var mainPrice = parseFloat(row.find('.unit-price').data('main-price'));
        var disPrice = parseFloat(row.data('dis-price'));
        
        var unitPrice = (qty >= 6) ? disPrice : mainPrice;
    ...
    function addNewRow(barcode, response) {
        var newRow = '<tr data-barcode="' + barcode + '" data-dis-price="' + response.dis_price + '">';
        ...
        newRow += '<td class="unit-price table-column" data-main-price="' + response.main_price + '">' + response.main_price + '</td>';
        var totalPrice = parseFloat(response.main_price);
